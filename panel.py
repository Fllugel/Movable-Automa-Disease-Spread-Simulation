...
class Panel:
    def __init__(self, width, height, grid):
        ...
        self.infection_period_input_box = pygame.Rect(width // 2 - 75, 230, 150, 30)
        ...
        self.infection_period_color = self.color_inactive
        ...
        self.active_infection_period = False
        ...
        self.infection_probability_text = str(grid.infection_probability)
        self.infection_radius_text = str(grid.infection_distance)
        self.infection_period_text = str(grid.infection_period)
    ...
    def handle_event(self, event):
        if event.type == pygame.MOUSEBUTTONDOWN:
            if self.cell_count_input_box.collidepoint(event.pos):
                ...
                self.active_infection_probability = False
                self.active_infection_period = False
            elif self.infection_period_input_box.collidepoint(event.pos):
                self.active_infection_period = not self.active_infection_period
                self.active_cell_count = False
                self.active_infected_count = False
                self.active_cell_speed = False
                self.active_infection_probability = False
            ...
            self.infection_period_color = self.color_active if self.active_infection_period else self.color_inactive
            ...
            if self.button.collidepoint(event.pos):
                try:
                    ...
                    infection_period = int(self.infection_period_text) if self.infection_period_text.isdigit() else self.grid.infection_period
                    ...
                    self.grid.infection_period = infection_period
                except ValueError:
        ...
        if event.type == pygame.KEYDOWN:
            ...
            elif self.active_infection_period:
                if event.key == pygame.K_BACKSPACE:
                    self.infection_period_text = self.infection_period_text[:-1]
                else:
                    self.infection_period_text += event.unicode
    ...
    def draw(self, screen):
        ...
        input_labels = ["Cell Count", "Infected Count", "Cell Speed", "Infection Probability", "Infection Radius", "Infection Period"]
        input_boxes = [
            ...
            (self.infection_period_input_box, self.infection_period_color, self.infection_period_text)
        ]
